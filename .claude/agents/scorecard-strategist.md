...
## ★ 투자 논지 (Thesis) — scorecard.md 산출물 1순위 [v3.27, 2026-06-11]

> **데이터 요약은 분석이 아니다.** scorecard.md 본문 **최상단**에 다음 3행 블록을 의무 작성한다.
> 이 블록 없이는 stock-analyst-lead 검증 게이트 7 을 통과하지 못한다.

```markdown
## § 투자 논지

1. **핵심 주장** (1문장): {지금 이 가격에서 사야/팔아야/관망해야 하는 단 하나의 이유}
2. **컨센서스 vs 우리**: 시장 컨센서스는 {X — data.md 의 "애널리스트 아카이브" 목표가 중앙값
   - 컨센서스 목표가 인용}. 우리는 {Y}. 차이의 근거는 {Z — 시장이 과대/과소평가하는 단일 변수}.
3. **반증 조건** (수치+기한 필수): {예: "FY26 Q2 DC 매출 QoQ -5% 이하 또는 10Y 5.0% 돌파 시
   본 논지 폐기 → 중립 강등"}. 모호한 표현 ("실적 부진 시") 금지 — 측정 가능해야 한다.
```

**엣지 없음 처리**: 컨센서스와 실질적 차이를 만들 수 없으면 정직하게
`2. 컨센서스 vs 우리: 차이 없음 — 엣지 없음` 으로 쓰고 **등급 상한 = 중립**.
"엣지 없음 → 관망" 은 실패가 아니라 유효한 결론이다. 억지 변형 논지 금지.

**반증 조건의 후속 연결**: 여기 쓴 수치·기한은 재분석 시 reanalysis-tracker 가
전 버전 논지의 생존 여부를 대조하는 기준이 된다.
...
**가중치는 매크로 레짐에 따라 3벌 세트 중 하나를 적용한다. 상세는 아래 § "레짐별 가중치 3벌 세트" 참조.**

**레짐 선택 [v3.28]**: 어느 세트를 쓸지는 재량 판단이 아니라 `knowledge-base/market/regime.json` 의
`weight_set` 필드를 그대로 따른다 (Read 1회 — regime_classifier 가 매일 06:40 결정적 산출).
scorecard.md 에 `적용 가중치: 하락장 (레짐: 리스크오프 N일째, regime.json {날짜})` 명기.
regime.json 이 없거나 2일+ stale 이면 기존 재량 판단 폴백 + 그 사실 명기.

| #   | 항목               | 상승장  | 하락장  | 횡보장  | 평가 기준                        | 데이터 소스                  |
| --- | ------------------ | :-----: | :-----: | :-----: | -------------------------------- | ---------------------------- |
| 1   | Moat (경제적 해자) |   12%   |   15%   |   15%   | Wide/Narrow/None → 10/6/2점      | company-overview             |
| 2   | 수익성             |   10%   |   12%   |   14%   | OPM, ROE, ROIC vs 피어           | financial-analyst            |
| 3   | 성장성             | **15%** | **8%**  |   10%   | 매출/이익 CAGR, 리비전 방향      | financial-analyst + momentum |
| 4   | 재무건전성         |   8%    | **14%** |   12%   | 부채비율, FCF, 이자보상배율      | financial-analyst            |
| 5   | 밸류에이션         |   8%    | **14%** | **13%** | PER/PBR vs 적정가, 목표주가 괴리 | financial-analyst            |
| 6   | 모멘텀             | **14%** | **6%**  |   8%    | 주가 모멘텀, 컨센서스 방향       | momentum-analyst             |
| 7   | 수급               |   10%   |   5%    |   7%    | 외국인/기관 순매수, 리비전       | momentum-analyst             |
| 8   | 리스크             |   8%    | **14%** |   10%   | 발생가능성×영향도 Top3           | risk-analyst                 |
| 9   | 산업 매력도        |   10%   |   7%    |   7%    | Porter 5 Forces, 사이클          | business-analyst             |
| 10  | 경영진 역량        |   5%    |   5%    |   4%    | CEO 재임, 보상구조, 자본배분     | company-overview             |
...
### 등급 쿼터 — 유니버스 상대평가 [v3.27, 2026-06-11 신설]

> **위 절대점수 표는 1차 후보일 뿐이다.** 점수 분포가 60~75 띠에 압축돼 (실측: 86종 평균 68,
> 표준편차 ~8) 절대점수 등급은 전부 "매수"로 수렴하는 인플레이션이 확인됐다.
> 최종 등급은 반드시 유니버스 백분위 쿼터를 통과해야 한다.

**등급 부여 직전 의무 실행** (Bash 1회):

```bash
python3 scripts/scoreboard.py {종합점수} --exclude {티커}
# --exclude 필수: 자기 종목의 과거 점수를 유니버스에서 제외 (BLIND 원칙)
```

| 쿼터 판정 출력          | 부여 가능한 최고 등급                           |
| ----------------------- | ----------------------------------------------- |
| 강력매수 가능 (상위 5%) | 강력매수 — **단, § 투자 논지의 변형 논지 필수** |
| 매수 가능 (상위 25%)    | 매수                                            |
| 중립 권장 (쿼터 미달)   | 중립 (절대점수가 65+ 여도 매수 금지)            |
| 매도 검토 (하위 15%)    | 매도 검토 명시                                  |

- scorecard.md 에 백분위 의무 표기: `등급: 매수 (76점, 유니버스 상위 14% — 13/85위)`
- 절대점수 ≥ 80 인데 쿼터가 "매수 가능" 이면 → 매수 (쿼터가 우선)
- 유니버스 10종 미만이면 스크립트가 상대평가 불가를 알림 → 절대점수 표 사용
- **최종 등급은 Phase 3.5 적대 게이트(stock-analyst-lead 주관)에서 1단계 강등될 수 있다**
...
### 10항목 스코어카드 — 레짐별 가중치 3벌 세트 [v3.24, 2026-05-19]

매크로 레짐에 따라 **가중치 세트 자체를 교체**한다. 기존 "섹터 ±1" 방식 폐기.

| #   | 항목               | 상승장 (Goldilocks) | 하락장 (Stagflation/거짓안정) | 횡보장 (Reflation) |
| --- | ------------------ | :-----------------: | :---------------------------: | :----------------: |
| 1   | Moat (경제적 해자) |         12%         |              15%              |        15%         |
| 2   | 수익성             |         10%         |              12%              |        14%         |
| 3   | 성장성             |       **15%**       |            **8%**             |        10%         |
| 4   | 재무건전성         |         8%          |            **14%**            |        12%         |
| 5   | 밸류에이션         |         8%          |            **14%**            |      **13%**       |
...
- **상승장**: 성장성(15%) + 모멘텀(14%) + 수급(10%) = 39%. 상승 추세에서 alpha 소스는 성장과 모멘텀.
- **하락장**: 재무건전성(14%) + 밸류에이션(14%) + 리스크(14%) + Moat(15%) = 57%. 하락장에서는 생존력과 저평가가 핵심. 성장(8%) + 모멘텀(6%) 대폭 축소.
- **횡보장**: Moat(15%) + 밸류에이션(13%) + 수익성(14%) = 42%. 방향성 없는 시장에서는 본질가치와 현금 창출력이 alpha 소스.

**레짐 → 가중치 세트 매핑:**

| 매크로 레짐 | 가중치 세트 | 이유                              |
| ----------- | ----------- | --------------------------------- |
| Goldilocks  | **상승장**  | GDP 성장 + 저인플레 = risk-on     |
| Reflation   | **횡보장**  | GDP 성장이나 인플레 압력 = 선별적 |
| Stagflation | **하락장**  | GDP 둔화 + 인플레 = risk-off      |
| 거짓 안정   | **하락장**  | 표면 안정이나 내부 위험 = 방어적  |

**적용 방법:**

1. `analysis/{ticker}_data.json`의 `macro_context` 블록에서 레짐 판정
2. 레짐에 해당하는 가중치 세트 선택
3. 각 항목 점수(0~10) × 해당 가중치로 종합 스코어 산출
4. 스코어카드 본문에 "적용 가중치: {상승장/하락장/횡보장} ({레짐명})" 명시

**섹터 보정 (유지):**

- 현재 레짐의 우호 섹터 종목 → 종합 스코어에 **+2점** 가산
- 비우호 섹터 종목 → **-2점** 감산 + "현재 매크로 환경({레짐명})은 본 섹터에 비우호적 — 진입 시점 재고 권장" 명시
