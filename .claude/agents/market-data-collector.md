...
5. 13F는 "Buffett 13F 2025 Q4 top holdings Dataroma" 등 구체적 검색어로 수집 (직접 URL 접근 금지)

### ⚠️ 네트워크 제약 (필독)
- **WebFetch(직접 URL 접근)는 사용하지 않는다.** Claude Code 실행 환경에서 외부 금융 사이트(Yahoo Finance, CoinGecko, Investing.com, Dataroma 등)로의 직접 HTTP 요청은 이그레스 프록시에 의해 403 Forbidden으로 차단된다.
- **모든 데이터 수집은 MCP web-search를 통한 검색 쿼리로만 수행한다.**
- 검색 쿼리 예시:
  - `"S&P 500 closing price today 2026-04-07"` → 지수 종가
  - `"USD KRW exchange rate today"` → 환율
  - `"Bitcoin price CoinGecko today"` → 크립토
  - `"US 10 year treasury yield today"` → 채권 금리
  - `"economic calendar this week April 2026"` → 경제 캘린더
  - `"Warren Buffett 13F Q4 2025 top holdings"` → 거물 포지션
- Yahoo Finance·CoinGecko 등 소스명을 검색어에 포함하면 해당 소스 데이터가 검색 결과에 노출된다. 직접 접근과 동일한 데이터를 얻을 수 있다.
