...
> ⚠️ 본 시스템은 **사용자 1인 전용 분석 도구**다 (다수 구독자 공유 아님 — 2026-08 방향 전환).
> 규율: 시장·논지 분석은 **"내 보유를 보기 전에" 독립적으로 판단**(포지션 편향 방지) → 개인 결정 레이어를 그 위에 얹는다.
> ⚠️ **프라이버시 가드:** 배포 사이트가 아직 공개 상태(인증 게이팅은 후작업 예정)이므로 **개인 보유·계좌 데이터를 배포 리포트 본문에 넣지 않는다.** 개인 처방은 `/내포트폴리오` 로 격리 (개인 보유 전면 연동은 게이팅 완료 후).
...
**★ 의사결정 레이어 (2026-08 신설 · v2 대중어/눈길 개정) — 주간 리포트 최상단 필수 (Step 4):** 레짐 헤더 직후·C-1 앞 `## 🧭 의사결정 레이어` 박스. 목적 = "이번 주 전체를 읽어야 하나"를 3초에 판별. 기존 C-1~C-10 본문은 그대로(요약이 본문 대체 아님).

**⚠️ 언어 규칙 — 주식 초보 기준 대중어 (필수):** 전문용어 금지 또는 즉시 풀이. 예: '경화 로테이션'→"돈이 주식에서 금·기름·비트코인으로 이동" / 'debasement 헤지'→"돈 가치 하락 대비 실물 피신" / '하우스뷰 상태 변경'→"우리 시장 판단 수정" / 'dry powder·SGOV'→"현금·단기국채로 실탄 보관" / 'SOX'→"반도체 지수" / '잭슨홀'→"미국 중앙은행장 연설". **한 항목 = 한 문장, 짧게.**

**① 맨 위 판정 배너 (핵심 — 3초 컷):** 🟢/🟡/🔴 신호등 + 큰 한 줄 판정으로 시작.

- 🟢 **"그냥 넘겨도 돼요"** (실질 변화 없음 — 대부분의 주) · 🟡 **"한 번 봐두세요"** (변화 있음) · 🔴 **"꼭 확인하세요"** (손절·급변 등 즉시 사안).

**② 본문 4줄 (대중어 라벨 + 이모지):**

1. 🔄 **뭐가 바뀌었나** — 지난주 대비 변화. 없으면 "특별한 변화 없어요".
2. 🎯 **나 뭐 해야 하나** — 기본값 **"급하게 사고팔 때 아님(그대로 유지)"**. 하드 트리거(레짐 전환·시장판단 수정·모델 목표 이탈)일 때만 행동 언급. **확신만으로 행동 표기 금지**(C-9 과신 교정 — 대부분 '유지'가 정답). ⚠️ **개인 보유 기준 행동 금지**(게이팅 전 — 시장/모델 수준만; 개인 triage 는 게이팅 후 `/내포트폴리오`).
3. 👀 **이것만 챙기세요** — 다음 주 최우선 이벤트 1개.
4. 📖 **더 볼까?** — 걸린 게 있으면 `→ C-N`만 콕 집어. 없으면 "나머지는 안 봐도 OK".
...
0. [v3.26] Bash: python3 scripts/score_recommendations.py 실행 →
   ...
   해석·교훈 도출만 담당.
   [v3.36 필수] `breakdown.by_confidence` (확신도별 적중률) 표를 리포트 본문에 그대로 인용하고,
   역전(높음 < 중간) 지속 시 원인 코멘트 1줄 + 다음 기간 높음 라벨 억제 방침을 명시한다.
   미인용 시 성과리뷰 미완성으로 간주.
...
## Step 8.5-B: 확신 라벨 산정 룰 [v3.36 — 역캘리브레이션 교정, 2026-07-02]

**배경 실측 (auto_scoring 2026-06-27)**: 확신 "높음" 적중 **14.3%** (2/14) < "중간" **60.0%** (9/16).
강하게 단정할 서사가 있는 아이디어일수록 이미 가격에 반영됐거나 반증을 무시한 경우다.
확신은 서사의 강도가 아니라 **아래 체크리스트 통과 개수로만** 부여한다 (자유 판단 금지):

| #   | 점검              | 통과 기준                                                                                            |
| --- | ----------------- | ---------------------------------------------------------------------------------------------------- |
| 1   | 반증 조건 구체성  | 가격/일자/지표 임계가 **숫자로** 근거 1줄에 포함 (예: "10Y 4.6% 상회 시 폐기")                       |
| 2   | 촉매 확정성       | 날짜 확정 이벤트 존재 (실적일·FOMC·판결·발표일)                                                      |
| 3   | 컨센서스 갭       | 컨센과 방향이 다르거나 시장 미반영 요소 명시 — **컨센 동조 아이디어는 "높음" 금지** (이미 가격 반영) |
| 4   | 과거 캘리브레이션 | auto_scoring.json `by_confidence` 에서 "높음" 적중률 ≥ 40% (역전 지속 중엔 미충족)                   |

- **4개 전부 충족 → "높음"** / 2~~3개 → "중간" / 0~~1개 → "낮음". "중간~높음" 같은 걸침 라벨도 높음 요건 적용.
- 표현 룰: 확신 높음이라도 단정문("반드시", "확실히", "틀림없이") 금지 — 항상 조건부("X 유지되는 한", "Y 하회 시 폐기").
- 기계 게이트: `scripts/check_confidence.py` 가 briefing_commit.sh 에서 자동 검사 —
  높음 계열 행의 반증 트리거 누락(R1), 최근 30일 높음 비율 25% 초과(R2), 역캘리브레이션 중 높음 남발(R3).
