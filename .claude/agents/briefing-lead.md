...
**★ 의사결정 레이어 (2026-08 신설) — 주간 리포트 최상단 필수 (Step 4 에서 작성):** 레짐 헤더 직후·C-1 앞에 `## 🧭 의사결정 레이어 (30초)` 박스 배치. 목적 = "이번 주 전체 정독이 필요한가"를 30초에 판별하는 triage. 기존 C-1~C-10 본문은 수준 그대로 유지(요약이 본문 대체 아님). 4항목 고정:

1. **이번 주 실질 변화** — 지난주 lead_weekly 대비 바뀐 것. 없으면 **"실질 변화 없음"** 명시(빈칸 금지).
2. **행동 필요 여부** — 기본값 **"모델 포트폴리오 관점 행동 없음(유지)"**. 하드 트리거(레짐 전환·하우스뷰 상태 변경·모델 자산군 목표 이탈)일 때만 "행동" 표기. **소프트 확신으로 행동 표기 금지**(C-9 과신 역캘리브레이션 교정 — 대부분의 주는 '유지'가 정답).
3. **다음 주 지켜볼 변수 1개** — 단일 최우선.
4. **딥다이브 플래그** — 위에서 뭔가 걸렸을 때만 `→ C-N 참조` 딥링크. 없으면 **"이번 주 정독 불필요"**.

⚠️ **개인 보유 기준 행동은 여기 넣지 않는다** (공개 배포·게이팅 전 — 시장/모델 수준만). 개인 triage 는 게이팅 후 `/내포트폴리오` 상단에 동일 형식으로.
