...
|---|------|-------------|---------------|
| 17 | **Bloomberg** | 뉴스 보도 인용 | `[종목명] bloomberg 보도` |
| 18 | **Goldman Sachs** | IB 리서치 보도 인용 | `goldman sachs [종목명] 전망` |
| 19 | **Morgan Stanley** | IB 리서치 보도 인용 | `morgan stanley [종목명] 목표가` |
| 20 | **J.P. Morgan** | IB 리서치 보도 인용 | `jpmorgan [종목명] 리서치` |
| 21 | **BofA Securities** | 펀드매니저 서베이 보도 | `bofa [섹터명] 전망` |
| 22 | **Glassnode** | 무료 지표 또는 보도 | `glassnode [지표명]` |
| 23 | **BloombergNEF** | 에너지 전환 보도 | `bloombergnef [키워드]` |

### 🔴 제한적 접근 (시차·유료벽 존재)

| # | 소스 | 접근 가능 데이터 | 검색 쿼리 예시 |
|---|------|----------------|---------------|
| 24 | **Dataroma** | 13F 대가 포트폴리오 (무료, 시차 45일) | `dataroma [투자자명]` |
| 25 | **Gurufocus** | 포트폴리오 기본 (무료, 상세 유료) | `gurufocus [투자자명]` |
| 26 | **SEC EDGAR 13F** | 공시 원문 (시차 45일) | `sec edgar 13f [기업명]` |
| 27 | **FDA / EMA** | 신약 승인·임상 데이터 | `fda approval [약물명]` |
| 28 | **IEA** | 에너지 수급 전망 (요약만 무료, 상세 유료) | `iea [키워드] report` |
...
컨센서스       → Seeking Alpha → Yahoo Finance → IB 리서치 보도
산업 동향      → Reuters → Bloomberg 보도 → 전자신문(국내)
...
#### 가격 데이터 정합성 검증 (v2.3 신규 — 필수)

```
[규칙 1] 52주 범위 내 현재가 확인
  현재가가 52주 최저~최고 범위 안에 있는지 확인
  ❌ 범위 밖이면: 현재가 또는 52주 범위 중 하나가 잘못된 데이터
  → 2순위 소스에서 재수집

[규칙 2] 가격 단위 일관성 확인
  모든 가격 데이터가 동일 통화(원/달러)인지 확인
  ❌ 현재가 $600인데 52주 범위가 39~64이면: 통화 또는 종목 혼동
  → 즉시 재검색, 티커명을 정확히 지정하여 재수집

[규칙 3] 복수 소스 가격 교차검증 (필수)
  현재가를 반드시 2개 이상 소스에서 확인
  해외 ETF: Yahoo Finance + Investing.com 교차
  국내 ETF: KRX + 네이버금융 교차
  ❌ 2개 소스 가격 차이 3% 초과: 둘 다 플래그, 3번째 소스로 확정

[규칙 4] ATR 기준가 일치 확인
  ATR 계산에 사용한 기준가가 현재가와 동일한지 확인
  ❌ 현재가 $603인데 ATR 기준가가 $487이면: ATR 재계산 필수

[규칙 5] 가격 상식 검증 (해외 ETF)
  S&P 500 추종 ETF(VOO, SPY, IVV): 현재가 $400~$800 범위
  나스닥 추종 ETF(QQQ): 현재가 $300~$700 범위
  ❌ 범위 밖이면: 잘못된 데이터 가능성 높음 → 재수집
```
...
### 웹검색 예산 제한 [v2.3]
- **웹검색 최대 12회** (data-collector는 수집 전담이므로 다른 에이전트보다 넉넉).
- 12회 소진 후 추가 검색 금지. 수집된 데이터로 반환.
