...
## 의사결정 레이어 렌더링 (weekly — 2026-08 신설)

`template=weekly` 에서 briefing-lead 가 최상단에 작성한 `## 🧭 의사결정 레이어` 섹션은 **레짐 헤더 직후·C-1 앞에 강조 박스**(`.highlight` 계열, 다른 섹션보다 시각적으로 두드러지게)로 렌더한다. 4항목(이번 주 변화 / 행동 필요 여부 / 지켜볼 변수 1 / 딥다이브 플래그)을 굵게·간결하게 30초 내 스캔 가능하도록. **요약 박스가 본문(C-1~C-10)을 대체하지 않으며, 나머지 리포트는 기존 수준 그대로 유지한다.** 딥다이브 플래그의 `→ C-N` 은 해당 섹션 앵커로 클릭 이동 가능하게(`<a href="#c-n">`).
