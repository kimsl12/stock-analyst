...
- launchd **16:15 KST** (`com.stockanalyst.algo-sync`) + watchdog (06:40/10:30) 이 파일 변경 감지 → commit/push → Cloudflare 미러(풀사이트) + Vercel(플래그 해제 후 본서버) 배포 → `/portfolio` 페이지 "알고 자동매매" 섹션에 표시. 일 3회 반영 (15:40 매매분은 16:15 슬롯).
...
## 10. 보호 종목 계약 (protected_holdings) — 2026-07-02 신설, 즉시 적용 요청

**사고**: 2026-06 말 엔진이 사용자 **수동 보유** 방어 슬리브 5종(SGOV·RSP·VIG·QUAL·VEA)을 전량 청산하고 BAC·AVGO 소수점을 편입했다. 결과: 채권/단기채 비중 6.05% → **0%**, 팩터·배당성장 슬리브 소멸 — 사용자 균형형 자산 배분이 붕괴됐다. 이는 하이브리드 정책의 **3채널 분리**(알고 채널은 엔진 소유 포지션만) 위반이다.

### 10.1 계약 파일 (엔진이 매 런 시작 시 읽기 — 필수)

**경로**: `algo-trading/data/protected_holdings.json` (schema `protected-holdings-v1`)

- `protected[].ticker` 에 등재된 종목은 **매도·축소·교체 후보 산정·임의 추가 매수 전부 금지**.
- 보유 상한 초과 시 "최저 스코어 종목 교체"(A3) 로직에서도 보호 종목은 후보에서 제외.
- 엔진의 관리 유니버스 = **엔진 자신이 편입한 포지션만** (algo_holdings.json 의 positions 와 일치해야 함).
- 목록은 종목분석 쪽이 관리 (사용자 수동 보유 변동 시 갱신). 엔진은 읽기 전용.

### 10.2 §9 역방향 보고 미구현 재촉

§9 계약(algo_holdings.json — 체결 직후 + 15:40 런 종료 시 Write)이 **2026-06-12 통지 후 미구현** 상태다
(`engine_status: not_live` 그대로). 그 사이 엔진은 실거래(5종 청산·2종 편입)를 수행했고, 보고 부재로
사용자가 청산 사실을 **6/25~7/1 스크린샷 대조에서야** 발견했다 — 이번 자산 배분 붕괴의 가시성 원인.
**§9 구현을 최우선으로 요청** — 구현 즉시 /portfolio "알고 자동매매" 섹션에 자동 표시된다 (종목분석 쪽 준비 완료).
