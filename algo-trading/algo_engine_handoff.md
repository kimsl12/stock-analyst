...
| #   | 항목               | 가중치 | 평가 기준                            | 데이터 소스         |
| --- | ------------------ | ------ | ------------------------------------ | ------------------- |
| 1   | Moat (경제적 해자) | 15%    | Wide/Narrow/None → 10/6/2점          | 사업구조, 경쟁 우위 |
| 2   | 수익성             | 12%    | OPM, ROE, ROIC vs 동종 업계          | 재무제표            |
| 3   | 성장성             | 12%    | 매출/이익 CAGR, 컨센서스 리비전 방향 | 재무제표 + 컨센서스 |
| 4   | 재무건전성         | 10%    | 부채비율, FCF, 이자보상배율          | 재무제표            |
| 5   | 밸류에이션         | 10%    | PER/PBR vs 적정가, 목표주가 괴리     | 재무제표 + 시장     |
| 6   | 모멘텀             | 10%    | 주가 모멘텀, 컨센서스 방향           | 기술적 지표         |
| 7   | 수급               | 8%     | 외국인/기관 순매수, 리비전           | 거래소 데이터       |
| 8   | 리스크             | 10%    | 발생가능성 x 영향도 Top 3            | 정성 평가           |
| 9   | 산업 매력도        | 8%     | Porter 5 Forces, 사이클 위치         | 산업 분석           |
| 10  | 경영진 역량        | 5%     | CEO 재임, 보상구조, 자본배분         | 공시                |
...
현재 매크로 환경에 따라 특정 항목 가중치가 +/- 조정됨:

- 스태그플레이션 환경 → 리스크(8), 재무건전성(4) 가중치 상향
- 성장 사이클 → 성장성(3), 모멘텀(6) 가중치 상향
- 금리 상승기 → 밸류에이션(5), 재무건전성(4) 가중치 상향
