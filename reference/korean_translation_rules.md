...
> v3.22 (2026-06-01): 측정 버그 3건 + 설계 모순 1건 수정. 88/88 미달 → 272/323 통과 (84%).
>
> **수정 사항**:
>
> - B1: `<script>` 블록 미제거 → JS 키워드(`document`, `var`) 영어 카운트되던 버그
> - B2: 멀티라인 `style="..."` 속성 미제거 → CSS 키워드(`px`, `flex`) leak
> - B3: 표준 약어(예외 2) 도 영어 카운트되어 룰 본문 예외와 모순
> - D1: 고유명사(예외 1) 도 영어 카운트되어 룰 본문 예외와 모순

## 적용 범위

분석·리포트 **본문**에만 적용 — `analysis/` lead 산출물 md + `reports/**/*.html`.
`knowledge-base/`·`knowledge-db/` 데이터 파일은 **적용 제외** (데이터 무결성 > 한글화 — 2026-06-01 audit P2-14 확정).
...
# 1. 본문 추출 — v3.22 정제 파이프라인 (B1+B2 수정)
#    sed 단일행은 멀티라인 <script>/<style>/style="..." 누락 → perl 멀티라인 사용.
...
# 2. 매핑 사전 영어 표현 (등급·시나리오) — 본문 안에 영문 잔류 검사
...
# 3. 한글 비중 80% 이상 — v3.22 (B3+D1 수정)
#    예외 1 (고유명사) + 예외 2 (표준 약어) + 파일 경로/슬러그를 분모에서 제외해야
#    룰 본문 정의("3가지 예외는 영문 유지")와 측정 로직이 일관됨.
...
TOTAL=$(echo "$BC" | grep -oE '[가-힣A-Za-z]' | wc -l | tr -d ' ')
...
- 전 리포트 323종 측정 결과: **272종 (84%) 통과**
- 분포: 90%+ 147 / 80~89% 125 / 70~79% 33 / 60~69% 11 / <60% 7
- 미달 신호 = 실제 미번역 콘텐츠 (예: `turnaround`, `backlog`, `watch` — 매핑 사전 적용 누락)
- 임계값 80% 유지 — 위 정제 적용 시 본문 콘텐츠 한글화 의도가 잘 측정됨.
