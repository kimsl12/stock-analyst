...
```
[규칙 O1] 소스 독립성 검증
  2개 소스가 일치하더라도 아래 조건이면 3번째 소스 필수:
  - 두 소스의 보도 시각 차이가 2시간 이내 (받아쓰기 의심)
  - 두 소스가 동일 통신사(AP, Reuters, Yonhap) 발 기사인 경우
  
  ✅ 독립 소스 예시: Bloomberg 자체 취재 + 로이터 자체 취재
  ❌ 비독립 예시: 매경 AP발 + 한경 AP발 → 실질 1개 소스

[규칙 O2] 1차 소스 확인 우선
  가능하면 공식 1차 소스로 소급:
  - 기업 실적 → IR 공시 / DART
  - 경제지표 → 발표 기관 원문 (Fed, BLS, 통계청)
  - 정책 → 정부 공식 보도자료
  
  1차 소스와 보도 내용이 다르면 → 보도 내용 기각, 1차 소스 채택
  1차 소스 접근 불가 시 → "2차 소스 기반" 명시

[규칙 O3] 발표 주체 vs 보도 주체 분리 표기
  ❌ [Bloomberg] 미국 실업률 3.9%
  ✅ [BLS 발표, Bloomberg 보도] 미국 실업률 3.9%
  
  ❌ [한경] 삼성전자 영업이익 15조원
  ✅ [삼성전자 잠정실적 공시, 한경 보도] 영업이익 15조원
  
  발표 주체가 불분명한 경우:
  ✅ [업계 추정, 전자신문 보도] 시장 규모 $50B
```
