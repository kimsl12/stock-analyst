...
| `scripts/algo_portfolio_sync.sh`        | launchd 16:15 + watchdog | 엔진이 쓴 `algo-trading/data/algo_holdings.json` (역방향 계약, handoff §9) 변경 감지 → commit/push + Vercel·Cloudflare 배포 → `/portfolio` "알고 자동매매" 섹션 반영. live + 2일 무보고 시 watchdog 경고 |
