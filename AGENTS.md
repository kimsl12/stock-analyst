...
| `scripts/algo_portfolio_sync.sh`        | launchd 16:15 + watchdog | 엔진이 쓴 `algo-trading/data/algo_holdings.json` (역방향 계약, handoff §9) 변경 감지 → commit/push + Vercel 배포(timeout 가드, /portfolio 는 본서버 단독) → "알고 자동매매" 섹션 반영. live + 2일 무보고 시 watchdog 경고 |
