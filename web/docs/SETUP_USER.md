...
## 1. Day 1 검증 — Supabase 매직 링크 실제 메일 수신

**목적:** Magic Link 인증이 실 운영에서 작동하는지 사용자 측에서 1회 검증.

**단계:**
1. https://stock-analyst-jungwon1.vercel.app/login 접속
2. `jungwon9402@gmail.com` 입력 → "매직 링크 받기" 클릭
3. 메일함 확인 (스팸함 포함, 1-2분 내 도착)
4. 메일 내 링크 클릭 → `/auth/callback` 자동 처리 → `/` 로그인 완료
5. 우측 상단 이메일 표시 + "로그아웃" 버튼 노출 확인

**실패 시:**
- 메일이 안 옴 → Supabase 대시보드 → Auth → Logs → 발송 로그 확인
- 콜백에서 거부 → §2.2 "URL Configuration" 확인
