#!/usr/bin/env python3
"""
재분석 자동 실행 — 2026-06-07 (회차 1, 10일 임계) — 10종 BLIND 일괄 생성
대상: XOM v5, VST v3, VRT v3, VIG v3, UNP v5, TLT v5, TIP v5, WMT v5, VZ v5, V v5
- BLIND 모드: 이전 v{N-1} 절대 read 안 함
- 콘텐츠는 종목별 specialist 에이전트가 작성한 analysis/{folder}_v{N}/_content.json 에서 읽음
- 가격은 analysis/{folder}_v{N}/data.json (사전 fetch_price.py 수집)
...
PLAN = {
    "GLD":  ("SPDRGoldShares",          "SPDR 골드셰어",        5, "2026-05-27"),
    "MUU":  ("DirexionDailyMU2X",       "Direxion MU 2X 레버리지 ETF", 2, "2026-05-27"),
    "MU":   ("Micron",                  "마이크론 테크놀로지",   5, "2026-05-27"),
    "PEP":  ("PepsiCo",                 "펩시코",               5, "2026-05-27"),
    "PG":   ("PG",                      "프록터앤갬블",         5, "2026-05-27"),
    "RSP":  ("InvescoSP500EqualWeight", "인베스코 S&P500 동일가중 ETF", 5, "2026-05-27"),
    "SAP":  ("SAP",                     "SAP",                  5, "2026-05-27"),
    "SCHD": ("SchwabUSDividendEquity",  "슈왑 미국 배당주 ETF", 5, "2026-05-27"),
    "TTE":  ("TotalEnergies",           "토탈에너지스",         5, "2026-05-27"),
    "TXN":  ("TexasInstruments",        "텍사스 인스트루먼츠",  5, "2026-05-27"),
}


def _fmt(v):
    """confidence 목표가 숫자 포맷 — 숫자면 천단위, 문자면 그대로"""
...
def main():
    ...
    print(f"=== 재분석 자동 실행 — {TODAY} BLIND — {len(only)}종 ===\n")
    ok, fail = [], []
    for ticker in only:
