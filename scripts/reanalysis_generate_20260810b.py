"""재분석 자동 실행 — 2026-08-10 (7일 임계, 상한 10, stock_update-2 슬롯, /재분석실행 7 20→cap10)
08-01 코호트 상장 10종(9일 경과, 티커 알파벳순 tiebreak). ANTHROPIC(75일) standing 제외.
- BLIND: 분석가(general-purpose 10종 병렬)가 _content.json 작성 (이전 v·HTML·timeline 미참조)
- 가격: analysis/{folder}_v{N}/data.json (fetch_price.py 수집, 종가 2026-08-07)
...
TODAY = "2026-08-10"
YYYYMMDD = "20260810"
RUN_MD = "20260810b_run"
THRESHOLD = 7

# ticker -> (folder_name, name_kr, next_v, prev_v_date)
PLAN = {
    "466100": ("클로봇",         "클로봇",         12, "2026-08-01"),
    "AMZN":   ("Amazon",         "아마존",         12, "2026-08-01"),
    "GOOGL":  ("Alphabet",       "알파벳",         12, "2026-08-01"),
    "GS":     ("GoldmanSachs",   "골드만삭스",     13, "2026-08-01"),
    "LNG":    ("CheniereEnergy", "셰니에르에너지", 5,  "2026-08-01"),
    "MELI":   ("MercadoLibre",   "메르카도리브레", 5,  "2026-08-01"),
    "NFLX":   ("Netflix",        "넷플릭스",       5,  "2026-08-01"),
    "NOW":    ("ServiceNow",     "서비스나우",     5,  "2026-08-01"),
    "PEP":    ("PepsiCo",        "펩시코",         12, "2026-08-01"),
    "QCOM":   ("Qualcomm",       "퀄컴",           5,  "2026-08-01"),
}
...
def main():
    ...
    print(f"=== 재분석 — {TODAY} ({THRESHOLD}일 임계, 상한 10, b회차) BLIND — {len(only)}종 ===\n")
    ok, fail = [], []
    for ticker in only:
