#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""재분석 데이터 수집 — 2026-08-27b (7일 임계, stock_update-2 슬롯, /재분석실행 7 20→cap10).
검출: timeline.json max-v date 기준 08-17 코호트(10일) 잔여 20종. 자매 10일런(01:00, f98d93c6)이
티커 오름차순 상위 10종(010120~AZN)을 처리하고 "나머지 20종(BABA~WMT)은 7일런 몫"으로 명시 반납.
그중 티커 오름차순 상위 10종(BABA~NVDA) → 본 런. 나머지 10종(PM~WMT)은 후속 7일런 몫 — 무중복.
yfinance 단일계열(WebSearch 미바인딩)로 리치 data.json 생성 → analysis/{ticker}_{folder}_v{nv}/data.json.
BRKB는 yfinance 심볼 BRK-B 로 오버라이드(저장 ticker/폴더는 BRKB 유지).
로직은 20260827_10day 검증본 재사용, PLAN + 심볼 오버라이드만 교체.
"""
...
PLAN = {
    "BABA": ("Alibaba",            "알리바바",              14, 13, "2026-08-17"),
    "BAC":  ("BankOfAmerica",      "뱅크오브아메리카",      14, 13, "2026-08-17"),
    "BRKB": ("BerkshireHathaway",  "버크셔해서웨이",        14, 13, "2026-08-17"),
    "DIA":  ("SPDRDJIA",           "SPDR 다우존스 ETF",     14, 13, "2026-08-17"),
    "GEV":  ("GEVernova",          "GE버노바",              12, 11, "2026-08-17"),
    "IWM":  ("iSharesRussell2000", "iShares 러셀2000 ETF",  14, 13, "2026-08-17"),
    "JPM":  ("JPMorgan",           "JP모건",                12, 11, "2026-08-17"),
    "KO":   ("CocaCola",           "코카콜라",              13, 12, "2026-08-17"),
    "LRCX": ("LamResearch",        "램리서치",              12, 11, "2026-08-17"),
    "NVDA": ("NVIDIA",             "엔비디아",              14, 13, "2026-08-17"),
}

SYMBOL_OVERRIDE = {"BRKB": "BRK-B"}


def yf_symbol(ticker):
    if ticker in SYMBOL_OVERRIDE:
        return SYMBOL_OVERRIDE[ticker]
