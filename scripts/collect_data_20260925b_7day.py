#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""재분석 데이터 수집 — 2026-09-25 (7일 임계, stock_update-2 슬롯, /재분석실행 7 20->cap10).
검출: 디스크 reports/ 최신 HTML 날짜 기준 7일+ 경과 (진실 소스, bootstrap AUTOGEN stale 확인).
  09-16 배치(8일 경과) 16종 중 상한 10종. SPACEX/ANTHROPIC(비상장·비표준) standing 제외.
대상 10종(전부 미국): XOM WMT VZ VST VRT VIG NFLX MELI GS GOOGL.
yfinance 단일계열(WebSearch 미바인딩)로 리치 data.json 생성. 09-25 10day 스크립트 재사용.
KR ATR nan 방어: history dropna 를 High/Low/Close 전체에 적용(본 회차는 미국주만)."""
...
PLAN = {
    "XOM":   ("ExxonMobil",                   "엑슨모빌",              17, 16, "2026-09-16"),
    "WMT":   ("Walmart",                      "월마트",                17, 16, "2026-09-16"),
    "VZ":    ("Verizon",                      "버라이즌",              17, 16, "2026-09-16"),
    "VST":   ("VistraCorp",                   "비스트라",              15, 14, "2026-09-16"),
    "VRT":   ("VertivHoldings",               "버티브홀딩스",          15, 14, "2026-09-16"),
    "VIG":   ("VanguardDividendAppreciation", "Vanguard 배당성장 ETF", 15, 14, "2026-09-16"),
    "NFLX":  ("Netflix",                      "넷플릭스",              10,  9, "2026-09-16"),
    "MELI":  ("MercadoLibre",                 "메르카도리브레",        10,  9, "2026-09-16"),
    "GS":    ("GoldmanSachs",                 "골드만삭스",            18, 17, "2026-09-16"),
    "GOOGL": ("Alphabet",                     "알파벳",                17, 16, "2026-09-16"),
}
    ...
    for ticker in only:
        ...
        try:
            ...
            print(f"[{ticker}] OK v{nv} {out['meta']['currency']}{q['current_price']} ({out['meta']['price_as_of']}) "
                  f"type={out['meta']['quote_type']} PE={out['valuation']['trailingPE']} fwdPE={out['valuation']['forwardPE']} "
                  f"mcap={q['marketCap']} atr={q['atr_14']} tgtMean={out['consensus']['targetMeanPrice']} -> {out_dir}/data.json")
            ok.append(ticker)
        except Exception as e:
