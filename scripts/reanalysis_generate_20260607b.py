#!/usr/bin/env python3
"""
재분석 자동 실행 — 2026-06-07 (회차 2, 7일 임계) — 10종 BLIND 일괄 생성
대상: 035420 NAVER v5, 066570 LG전자 v2, ASML v5, AZN v5, BABA v5,
      BAC v5, BRKB v5, C v5, CAT v5, DIS v5 (전원 2026-05-27 분석, 11일 경과)
- BLIND 모드: 이전 v{N-1} 절대 read 안 함 (콘텐츠는 종목별 에이전트가 _content.json 작성)
- 가격: data.json (사전 fetch_price.py 수집, 2026-06-05 종가)
...
PLAN = {
    "035420": ("035420_NAVER", "NAVER", 5, "2026-05-27"),
    "066570": ("066570_LG전자", "LG전자", 2, "2026-05-27"),
    "ASML":   ("ASML_ASML", "ASML", 5, "2026-05-27"),
    "AZN":    ("AZN_AstraZeneca", "아스트라제네카", 5, "2026-05-27"),
    "BABA":   ("BABA_Alibaba", "알리바바", 5, "2026-05-27"),
    "BAC":    ("BAC_BankOfAmerica", "뱅크오브아메리카", 5, "2026-05-27"),
    "BRKB":   ("BRKB_BerkshireHathaway", "버크셔 해서웨이 B", 5, "2026-05-27"),
    "C":      ("C_Citigroup", "씨티그룹", 5, "2026-05-27"),
    "CAT":    ("CAT_Caterpillar", "캐터필러", 5, "2026-05-27"),
    "DIS":    ("DIS_Disney", "디즈니", 5, "2026-05-27"),
}
...
def make_md_files(ticker, folder, kr, nv, prev_date, c, d):
    out_dir = f"analysis/{folder}_v{nv}"
    cur = d.get("currency", "$")
    prevv = nv - 1
...
> **재분석 v{nv}** — 이전 v{prevv} 비교 미포함 (Phase 2 reanalysis_runs/{YYYYMMDD}_run.md 참조)
> **BLIND 모드 — 이전 v{prevv} read 0건**
...
> 이전 v{prevv} ({prev_date}) 와의 차이는 `analysis/_reanalysis_runs/{YYYYMMDD}_run.md` 비교표 참조.
"""
...
def make_html_report(ticker, folder, kr, nv, prev_date, c, d):
    ...
    report_data = {
        "ticker": ticker,
        "name": f"{kr} ({folder.split('_', 1)[1] if '_' in folder else folder})",
        "date": TODAY,
        "score": c["score"],
        "grade": c["grade"],
        ...
        "custom_sections": [
            ...
            },
            {
                ...
                "content": (
                    ...
                    f"- **임계 경과**: 11일 (7일 임계 회차2)\n"
                    f"- **회차 보고**: `analysis/_reanalysis_runs/{YYYYMMDD}_run.md`"
                )
            }
        ]
    }
    output_path = f"reports/{ticker}_{folder.split('_', 1)[1] if '_' in folder else folder}_{YYYYMMDD}.html"
...
def main():
    ...
    print(f"=== 재분석 자동 실행 — {TODAY} BLIND 회차2 — {len(only)}종 ===\n")
    ok, fail = [], []
    for ticker in only:
        folder, kr, nv, prev_date = PLAN[ticker]
        cpath = f"analysis/{folder}_v{nv}/_content.json"
        dpath = f"analysis/{folder}_v{nv}/data.json"
