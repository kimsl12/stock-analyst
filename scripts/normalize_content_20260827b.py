#!/usr/bin/env python3
"""_content.json 스키마 정규화 — 2026-08-27b 7일 회차(stock_update-2, /재분석실행 7 20→cap10) — 멱등.
consensus/risks/scorecard/fragile/confidence 표준화 + 필수 키 보증 (generator KeyError 방지).
로직은 20260827 검증본 재사용, PLAN 만 08-17 코호트 잔여 티커 오름차순 상위 10종(BABA~NVDA)으로 교체."""
import json, os

PLAN = {
    "BABA": "Alibaba_v14", "BAC": "BankOfAmerica_v14",
    "BRKB": "BerkshireHathaway_v14", "DIA": "SPDRDJIA_v14",
    "GEV": "GEVernova_v12", "IWM": "iSharesRussell2000_v14",
    "JPM": "JPMorgan_v12", "KO": "CocaCola_v13",
    "LRCX": "LamResearch_v12", "NVDA": "NVIDIA_v14",
}
SEPS = ["—", "–", "·", ":", "：", " - "]
...
def main():
    ...
    for t, suf in PLAN.items():
        ...
        cur_price = (lambda dd: (dd.get("quote") or {}).get("current_price") or dd.get("current_price") or 0)(json.load(open(dp, encoding="utf-8")))
