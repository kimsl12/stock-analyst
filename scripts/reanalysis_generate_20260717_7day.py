#!/usr/bin/env python3
"""
재분석 자동 실행 — 2026-07-17 (7일 임계, 상한 20→10 클램핑, stock_update-2 슬롯)
대상 10종 (경과일 desc, 동률 알파벳순, ANTHROPIC 비상장 standing 제외):
  10일(2026-07-07): TM v8, TMUS v10, TQQQ v10(ETF3x), TSM v9, TTE v9, TXN v9, VOO v9(ETF)
  9일(2026-07-08 알파벳 상위 3): 035420 NAVER v10, 066570 LG전자 v7, CEG v8
- BLIND: general-purpose 에이전트가 _content.json 작성 (이전 v·HTML·timeline 미참조)
- 가격: analysis/{folder}_v{N}/data.json (사전 fetch_price.py 수집, 종가 2026-07-16)
- 출력: 종목당 6개 MD + scorecard + HTML 리포트
- 회차 보고: analysis/_reanalysis_runs/20260717_run_7day.md (메인이 별도 작성)
...
TODAY = "2026-07-17"
YYYYMMDD = "20260717"
RUN_MD = "20260717_run_7day"
THRESHOLD = 7

# ticker -> (folder_name, name_kr, next_v, prev_v_date)
PLAN = {
    "TM":     ("Toyota",               "도요타",              8,  "2026-07-07"),
    "TMUS":   ("TMobile",              "T-모바일",            10, "2026-07-07"),
    "TQQQ":   ("ProSharesUltraProQQQ", "프로셰어즈 나스닥100 3배", 10, "2026-07-07"),
    "TSM":    ("TSMC",                 "TSMC",                9,  "2026-07-07"),
    "TTE":    ("TotalEnergies",        "토탈에너지스",        9,  "2026-07-07"),
    "TXN":    ("TexasInstruments",     "텍사스인스트루먼트",  9,  "2026-07-07"),
    "VOO":    ("VanguardSP500",        "뱅가드 S&P500",       9,  "2026-07-07"),
    "035420": ("NAVER",                "네이버",              10, "2026-07-08"),
    "066570": ("LG전자",               "LG전자",              7,  "2026-07-08"),
    "CEG":    ("ConstellationEnergy",  "컨스텔레이션에너지",  8,  "2026-07-08"),
}
...
def make_html_report(ticker, folder, kr, nv, prev_date, c, d):
    ...
    output_path = f"reports/{ticker}_{folder}_v{nv}_{YYYYMMDD}.html"
