#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""재분석 HTML/MD 생성 — 2026-08-25 (7일 임계, 상한 10, stock_update-2 슬롯, /재분석실행 7 20→10).
08-15 코호트(10일) 중 스킬 정렬 상위 10종 (ANTHROPIC 비상장 standing 제외).
- BLIND: 분석가(Agent 10종 병렬)가 _content.json 작성 (이전 v·HTML·timeline 미참조)
- 가격/펀더멘털: analysis/{folder}_v{N}/data.json (collect_data_20260825b_7day.py, yfinance 리치 수집)
- extra_kpis(정량 KPI 표)는 data.json에서 메인이 중앙 주입 → 숫자 전사 오류 차단
- 통화 인식(KRW/USD): 컨센 평균목표·FCF 등 심볼 자동 · 한국 티커는 D/E·FCF 스케일 혼동 방지 위해 억제
- 배당수익률 아티팩트(SK 9%·LG 69%·BWXT 68%·CCJ 17%·CEG 63% 등 yfinance 스케일 오류) → DUK·ETN만 표기
...
PLAN = {
    "000660": ("SK하이닉스",           "SK하이닉스",         "SK Hynix",             9,  "2026-08-15"),
    "035420": ("NAVER",               "NAVER",             "NAVER",                14, "2026-08-15"),
    "066570": ("LG전자",              "LG전자",            "LG Electronics",       11, "2026-08-15"),
    "BWXT":   ("BWXTechnologies",     "BWX테크놀로지스",    "BWX Technologies",     8,  "2026-08-15"),
    "CCJ":    ("Cameco",              "카메코",             "Cameco",               8,  "2026-08-15"),
    "CEG":    ("ConstellationEnergy", "컨스텔레이션에너지", "Constellation Energy", 12, "2026-08-15"),
    "DUK":    ("DukeEnergy",          "듀크에너지",         "Duke Energy",          8,  "2026-08-15"),
    "ETN":    ("Eaton",               "이튼",               "Eaton",                12, "2026-08-15"),
    "KTOS":   ("KratosDefense",       "크라토스디펜스",     "Kratos Defense",       13, "2026-08-15"),
    "MLSS":   ("MilestoneScientific", "마일스톤사이언티픽", "Milestone Scientific", 13, "2026-08-15"),
}

KR_TICKERS = {"000660", "035420", "066570"}
# 배당수익률 진짜인 티커만 표기 — 나머지는 yfinance 스케일 아티팩트
KEEP_DIV = {"DUK", "ETN"}
...
def make_md_files(ticker, folder, kr, nv, prev_date, c, d):
    ...
    pdate = d.get("meta", {}).get("price_as_of", "2026-08-21")
    conf = c["confidence"]
        ...
        "custom_sections": [
            ...
            },
            {
                ...
                "content": (
                    ...
                    f"- **데이터**: yfinance 단일계열({d.get('meta',{}).get('price_as_of','2026-08-21')} 종가)\n"
