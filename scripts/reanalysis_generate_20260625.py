#!/usr/bin/env python3
"""
재분석 자동 실행 — 2026-06-25 (7일 임계, 상한 20→10 클램핑, stock_update-2 슬롯)
대상 10종: AVGO v8, ASTS v7, AMZN v7, AMAT v8, AGG v6, ABBV v6, 035720 v7, 034020 v7, 012450 v7, 010120 v8
- BLIND 모드: 이전 v{N-1} 절대 read 안 함 (general-purpose 에이전트가 _content.json 작성)
- 가격은 analysis/{folder}_v{N}/data.json (사전 fetch_price.py 수집, 종가 2026-06-24)
- 출력: 종목당 6개 MD + scorecard + HTML 리포트
- 회차 보고: analysis/_reanalysis_runs/20260625_run.md (메인이 별도 작성)
"""
import json, os, sys
        ...
        "custom_sections": [
            ...
            },
            {
                ...
                "content": (
                    ...
                    f"- **임계**: 7일 (상한 20→10종 클램핑, stock_update-2 슬롯, {TODAY})\n"
...
def main():
    ...
    print(f"=== 재분석 — {TODAY} (7일 임계, 상한 10) BLIND — {len(only)}종 ===\n")
    ok, fail = [], []
    for ticker in only:
