#!/usr/bin/env python3
"""
재분석 자동 실행 — 2026-06-08 (회차 2, 7일 임계) — 10종 BLIND 일괄 생성
...
PLAN = {
    "T":      ("ATT",            "AT&T",           5, "2026-05-27"),
    "010120": ("LSELECTRIC",     "LS일렉트릭",      6, "2026-05-28"),
    "034020": ("두산에너빌리티",  "두산에너빌리티",   5, "2026-05-28"),
    "035720": ("카카오",          "카카오",          5, "2026-05-28"),
    "BA":     ("Boeing",         "보잉",            5, "2026-05-28"),
    "GE":     ("GEAerospace",    "GE 에어로스페이스", 5, "2026-05-28"),
    "GS":     ("GoldmanSachs",   "골드만삭스",       6, "2026-05-28"),
    "HD":     ("HomeDepot",      "홈디포",          5, "2026-05-28"),
    "HSBC":   ("HSBC",           "HSBC 홀딩스",      5, "2026-05-28"),
    "JNJ":    ("JohnsonJohnson", "존슨앤드존슨",     5, "2026-05-28"),
}
...
> **재분석 v{nv}** — 이전 v{prevv} 비교 미포함 (Phase 2 reanalysis_runs/{YYYYMMDD}_run2.md 참조)
> **BLIND 모드 — 이전 v{prevv} read 0건**
...
> 이전 v{prevv} ({prev_date}) 와의 차이는 `analysis/_reanalysis_runs/{YYYYMMDD}_run2.md` 비교표 참조.
"""
        ...
        "custom_sections": [
            ...
            },
            {
                ...
                "content": (
                    ...
                    f"- **임계 경과**: 11~12일 (7일 임계 회차2)\n"
                    f"- **회차 보고**: `analysis/_reanalysis_runs/{YYYYMMDD}_run2.md`"
                )
            }
        ]
    }
...
def main():
    ...
    print(f"=== 재분석 자동 실행 — {TODAY} 회차2 BLIND — {len(only)}종 ===\n")
    ok, fail = [], []
    for ticker in only:
