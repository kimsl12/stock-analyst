#!/usr/bin/env python3
"""_content.json 스키마 정규화 (2026-07-25 10일 회차, /재분석실행 10 10) — 멱등.
consensus/risks/scorecard/fragile/confidence 표준화 + 필수 키 보증 (generator KeyError 방지)."""
import json, os

PLAN = {
    "052690": "한전기술_v4",
    "BWXT":   "BWXTechnologies_v5",
    "DUK":    "DukeEnergy_v5",
    "LQD":    "iSharesInvestmentGradeCorpBond_v4",
    "MLSS":   "MilestoneScientific_v10",
    "PLTR":   "Palantir_v11",
}
SEPS = ["—", "–", "·", ":", "：", " - "]
    ...
    for r in risks or []:
        if isinstance(r, dict):
            ...
            out.append({"name": name.strip()[:80], "level": "중", "impact": "중",
                        "desc": desc.strip()})
    # 최소 3개 보증 (generator 가 risks[0], risks[1] 참조)
    while len(out) < 3:
...
def norm_scorecard(items):
    ...
    # 10개로 맞춤 (초과 시 앞 10개, 부족 시 5.0 패딩)
    if len(rows) > 10:
        rows = rows[:10]
    while len(rows) < 10:
