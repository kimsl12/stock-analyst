#!/usr/bin/env python3
"""
재분석 자동 실행 — 2026-06-20 run2 (7일 임계, 상한 20→10 클램핑, stock_update-2 슬롯)
대상 10종: BHP v7, ANET v5, ADBE v7, 466100 v6, 329180 v7, 009150 v6, JPM v5, HOOD v6, COST v7, COIN v7
- BLIND 모드: 이전 v{N-1} 절대 read 안 함 (specialist 에이전트가 _content.json 작성)
- 가격은 analysis/{folder}_v{N}/data.json (사전 fetch_price.py 수집, 종가 2026-06-18/19)
- 출력: 종목당 6개 MD + scorecard + HTML 리포트
- 회차 보고: analysis/_reanalysis_runs/20260620_run2.md (메인이 별도 작성)
"""
import json, os, sys
...
TODAY = "2026-06-20"
YYYYMMDD = "20260620"
RUN_MD = "20260620_run2"

# ticker -> (folder_name, name_kr, next_v, prev_v_date)
PLAN = {
    "BHP":    ("BHPGroup",        "BHP그룹",          7, "2026-06-12"),
    "ANET":   ("AristaNetworks",  "아리스타네트웍스", 5, "2026-06-12"),
    "ADBE":   ("Adobe",           "어도비",           7, "2026-06-12"),
    "466100": ("클로봇",          "클로봇",           6, "2026-06-12"),
    "329180": ("HD현대중공업",    "HD현대중공업",     7, "2026-06-12"),
    "009150": ("삼성전기",        "삼성전기",         6, "2026-06-12"),
    "JPM":    ("JPMorgan",        "JP모간",           5, "2026-06-11"),
    "HOOD":   ("Robinhood",       "로빈후드",         6, "2026-06-12"),
    "COST":   ("Costco",          "코스트코",         7, "2026-06-12"),
    "COIN":   ("Coinbase",        "코인베이스",       7, "2026-06-12"),
}
...
def main():
    ...
    print(f"=== 재분석 run2 — {TODAY} (7일 임계, 상한 10) BLIND — {len(only)}종 ===\n")
    ok, fail = [], []
    for ticker in only:
