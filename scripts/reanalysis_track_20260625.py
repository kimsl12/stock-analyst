#!/usr/bin/env python3
"""
재분석 변화 추적 — 2026-06-25 (Phase 2)
신규 v{N} _content.json vs 직전 v{N-1} scorecard.md 비교 → analysis/_reanalysis_runs/20260625_run.md
read-only 비교 (분석 영향 없음).
"""
import json, os, re

TODAY = "2026-06-25"
RUN_MD = "analysis/_reanalysis_runs/20260625_run2.md"

# ticker -> (folder, kr, next_v, prev_v_date)
PLAN = [
    ("AVGO",   "Broadcom",                   "브로드컴",            8, "2026-06-16"),
    ("ASTS",   "ASTSpaceMobile",             "AST스페이스모바일",   7, "2026-06-16"),
    ("AMZN",   "Amazon",                     "아마존",              7, "2026-06-16"),
    ("AMAT",   "AppliedMaterials",           "어플라이드머티리얼즈", 8, "2026-06-16"),
    ("AGG",    "iSharesCoreUSAggregateBond", "iShares미국종합채권",  6, "2026-06-16"),
    ("ABBV",   "AbbVie",                     "애브비",              6, "2026-06-16"),
    ("035720", "카카오",                     "카카오",              7, "2026-06-16"),
    ("034020", "두산에너빌리티",             "두산에너빌리티",      7, "2026-06-16"),
    ("012450", "한화에어로스페이스",         "한화에어로스페이스",  7, "2026-06-16"),
    ("010120", "LSELECTRIC",                 "LS일렉트릭",          8, "2026-06-16"),
]
...
# ── run.md 작성
lines = []
lines.append(f"# 재분석 실행 결과 — {TODAY} (run2)")
lines.append("")
lines.append(f"**임계 기준:** 7일 / 상한 20→10종 클램핑 (stock_update-2 슬롯) | **처리:** 성공 10 / 스킵 0")
lines.append("**모드:** BLIND 재분석 (각 종목 이전 v{N-1} read 0건 — 독립 판단). 대상: 9일 경과 2026-06-16 분석분 10종.")
lines.append("> 같은 날 10일-런(staock_update)이 USMV 1종을 별도 처리(`20260625_run.md`). 본 run2 는 7일-런 10종.")
lines.append("")
lines.append("---")
lines.append("")
...
lines.append("| ---- | ------ | --------------- | ---- | ----------- | ----------- | --- | --------- | --------- | --------- | ----------------- | ----------------- |")
for r in rows:
    lines.append(f"| {r['ticker']} | {r['kr']} | v{r['nv']-1} / {r['prev_date']} | 9일 | {r['old_score']} | {r['new_score']} | {r['d']} | {r['old_grade']} | {r['new_grade']} | {r['chg']} | {r['old_tp']} | {r['new_tp']} |")
lines.append("")
lines.append("---")
lines.append("")
