#!/usr/bin/env python3
"""
재분석 자동 실행 — 2026-07-22 (10일 임계, 상한 10, staock_update 슬롯)
대상 3종 (모두 ETF, 2026-06-12 분석 → 40일 경과):
  QUAL v2, SGOV v2, VEA v2
- 검출 경위: timeline.json 사각지대(v 접미사 없는 폴더 → timeline 미생성 → bootstrap 미등재)
  교정(cleanup_reanalysis --tickers) 후 신규 검출. 기존 폴더는 _v1 으로 리네임.
- BLIND: 분석가가 _content.json 작성 (이전 v1·HTML·timeline 미참조)
- 가격: analysis/{folder}_v2/data.json (fetch_price.py 수집, 종가 2026-07-21)
- 출력: 종목당 6개 MD + scorecard + HTML 리포트
- 제외: ANTHROPIC(56일)·SPACEX(41일) — 비상장·비표준 standing 제외
"""
import json, os, sys
...
PLAN = {
    "QUAL": ("iSharesMSCIQuality",       "iShares MSCI USA 퀄리티 팩터 ETF", 2, "2026-06-12"),
    "SGOV": ("iShares0-3MonthTreasury",  "iShares 0-3개월 미국채 ETF",       2, "2026-06-12"),
    "VEA":  ("VanguardDevelopedMarkets", "뱅가드 선진국(미국제외) ETF",      2, "2026-06-12"),
}
...
def make_md_files(ticker, folder, kr, nv, prev_date, c, d):
    ...
    company_md = f"""# {kr} ({ticker}) — 상품개요 & 구조적 우위 (BLIND v{nv} 재분석)
...
"""
    ...
    financial_md = f"""# {kr} ({ticker}) — 펀드 재무 분석 (BLIND v{nv})
...
"""
    ...
    business_md = f"""# {kr} ({ticker}) — 시장/매크로 분석 (BLIND v{nv})
...
def make_html_report(ticker, folder, kr, nv, prev_date, c, d):
    ...
    report_data = {
        ...
        "asset_type": c.get("asset_type", "ETF"),
        ...
        "custom_sections": [
            ...
            },
            {
                ...
                "content": (
                    ...
                    f"- **검출 경위**: timeline.json 사각지대 교정으로 신규 검출 (40일 경과)\n"
