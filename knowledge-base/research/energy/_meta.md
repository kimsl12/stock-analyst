---
sector: energy
created: 2026-05-12
last_updated: 2026-08-08
status: active
related_industry_kb:
  - knowledge-base/industry/energy.md
  - knowledge-base/industry/capex.md
l1_index_count: 18
l2_summary_count: 7
l3_deep_dive_count: 1
deep_dive_priority: 2
---

# Research Meta — Energy (에너지)

> capex 19종 중 원자력/SMR (CEG·VST·SMR·OKLO·CCJ·BWXT) + 미국 유틸 (DUK·SO·NEE) 9종 영향. DC 전력 수요 $720B 그리드 CapEx 직접 베타.

## 현재 thesis

- **DC 전력 수요 thesis 격상**: IEA Electricity 2026 (4월) — DC 전력 2030년 2배·AI DC 3배, 2025년 미국 전력 수요 증가의 약 50%가 DC. 하이퍼스케일러 CapEx $400B → 2026년 +75% 추가. (L2: dc_power_iea_electricity)
- **NRC 라이선싱 패러다임 전환**: Part 53 발효(4/29) — PRA·SRE 기반 + 25,000명+ 고밀도 입지 + 제조지 연료 주입·운반·GLRO 신설. SMR/AP1000 양산 일정 2년 단축 시나리오 가능. (L2: nrc_part53_smr_licensing)
- **DOE 자금 트랙 가시화 + 마이크로원자로 별도 룰**: 에너지 장관 의회 증언(4월) — 첫 5~10기 신규 원전 DOE LPO 대출 거의 확정. SMR·CEG·VST·BWXT 자금 리스크 완화. 5/1 Federal Register 마이크로원자로 라이선싱 룰 별도 게재. (L2: doe_loan_5_10_new_reactors)
- **SMR 연료 공급망 첫 가시화**: NRC TRISO-X X-energy 연료 시설 5월 내 최종 라이선스 예상 — HALEU/TRISO 미국 자체 양산급 첫 사례, SMR 양산 단일 최대 병목 해소. (L2: triso_x_xenergy_fuel_facility)

## Key Uncertainties

- SMR 양산 시점 (NRC ESP 결정 2026 H2 5~10기 → 2028 가동 vs 2030 가동)
- DC 전력 수요 vs 신재생 + 가스 + 원자력 mix 비율 (IEA WEO 시나리오)
- 미국 그리드 CapEx 진행률 (IRA + FERC 인터커넥션 개혁)
- 우라늄 가격 사이클 (CCJ 베타) — 카자흐스탄 공급 + 러시아 제재
- 유럽 에너지 가격 (LNG vs 러시아 가스 + 원전 재가동)
- 중국 원전 양산 (CGN/CNNC) 글로벌 시장 점유율 변화

## 카탈리스트 캘린더

| 분기    | 이벤트                                | 출처 유형   |
| ------- | ------------------------------------- | ----------- |
| 2026 Q2 | NRC SMR licensing reform 결과         | Policy      |
| 2026 Q3 | IEA WEO 2026 발표 (10월)              | White Paper |
| 2026 Q4 | DOE Loan Programs Office disbursement | Policy      |
| 2026 H2 | SMR 5~10기 ESP 결정                   | Policy      |
| 2027 H1 | TerraPower Natrium 가동 일정 검증     | Filing      |

## L1 인덱스 카운트

- 학술: 0
- 씽크탱크/백서: 4
- 컨퍼런스: 0
- 규제: 4

## L2 월간 요약 목록

- 2026-08 `smr_commercialization_datacenter_ppa_202608.md` — Yale CEF SMR 상업화: FOAK→NOAK 원가 -50% + 2026 NRC 첫 상업 건설허가 2건(TerraPower Natrium 상반기)·NuScale 77MWe SDA + 하이퍼스케일러 10GW+ PPA(Amazon·Meta·Google·TVA) + IEA DC 1,100 TWh(+18%)
- 2026-06 `oklo_aurora_doe_pilot_criticality_202606.md` — Oklo Aurora DOE Reactor Pilot 7/4/2026 임계 목표 + DOE OTA·NSDA·PDSA + NRC 우회 가속 트랙
- 2026-05 `iea_key_questions_energy_ai_202605.md` — IEA Key Questions on Energy and AI (하이퍼스케일러 CapEx $400B+, +75%, 그리드 lead time 7~10년)
- 2026-05 `triso_x_xenergy_fuel_facility_202605.md` — NRC TRISO-X X-energy 연료 시설
- 2026-04 `dc_power_iea_electricity_202604.md` — IEA Electricity 2026 DC 전력 2배·AI DC 3배
- 2026-04 `nrc_part53_smr_licensing_202604.md` — NRC Part 53 발효 (Risk-Informed Technology-Inclusive)
- 2026-04 `doe_loan_5_10_new_reactors_202604.md` — DOE LPO 5~10기 신규 원전 대출

## L3 분기 Deep Dive 이력

- 2026 Q2 (2026-05-12) — `reports/research/energy_2026Q2.html`
  - 핵심: DC 전력 2030 2배 + SMR Part 53 + DOE 5~10기 + TRISO-X + CapEx 직접 베타
  - L2 5건 통합 + capex.md + energy.md + geopolitics.md 교차
  - 인용 15건 / 길이 61KB / debate-card 4건 / contrarian-card 3건 / 트래킹 지표 10개 / 종목 매트릭스 13종
- (예정) `reports/research/energy_2026Q3.html` — 2026-08 첫 일요일 발행 목표

## 분석 에이전트 활용 가이드

- 유틸 종목 (DUK, SO, NEE) → IEA WEO + DOE 그리드 CapEx L2
- 원전 종목 (CEG, VST) → NRC + ANS conference + 우라늄 가격 L2
- SMR (SMR·OKLO·BWXT) → NRC licensing + DOE loans + arXiv physics.acc-ph L2
- 우라늄 (CCJ) → 카자흐스탄 공급 + 러시아 제재 정책 L2
- 신재생 (FSLR, NEE) → IRA disbursement + LCOE NBER L2

## 관련 KB 교차 참조

- `knowledge-base/industry/energy.md`
- `knowledge-base/industry/capex.md` — 그리드 $720B
- `knowledge-base/macro/tech_breakthrough.md` — SMR/fusion 기술 단계
- `knowledge-base/macro/geopolitics.md` — 우라늄·LNG 지정학
