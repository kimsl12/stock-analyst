---
sector: auto
created: 2026-05-12
last_updated: 2026-07-25
status: active
related_industry_kb:
  - knowledge-base/industry/auto.md
  - knowledge-base/industry/battery.md
l1_index_count: 4
l2_summary_count: 1
l3_deep_dive_count: 0
deep_dive_priority: 10
---

# Research Meta — Auto / Mobility (자동차·EV·배터리)

> 5섹터 → 10섹터 KB 확장 Phase 1-A 골격. EV vs ICE 전환 + 중국 EV 글로벌 침투 + LFP vs NCM 점유율 + 자율주행 L4 + IEEPA 자동차 관세 + 차량용 반도체 6축 교차 섹터. 반도체 섹터와 직접 연결 (차량용 SoC·AI 추론칩).

## 현재 thesis (research-curator weekly/monthly 갱신)

- (L1 첫 수집 후 보강 예정 — 본 섹션은 status=active 전환 시 작성)

## 종목 매핑 (커버 유니버스)

| 종목                | 카테고리    | 핵심 베타         |
| ------------------- | ----------- | ----------------- |
| TSLA (Tesla)        | EV 리더     | EV·FSD·로보택시   |
| GM (General Motors) | 전통 자동차 | EV 전환·미국 SAAR |
| F (Ford)            | 전통 자동차 | F-150·EV·관세     |
| TM (Toyota)         | 일본 자동차 | 하이브리드·글로벌 |
| RIVN (Rivian)       | EV 신생     | 픽업·배달밴       |
| 005380 현대차       | 한국 자동차 | EV·SUV·미국 수출  |
| 000270 기아         | 한국 자동차 | EV·SUV·동남아     |
| HMC (Honda)         | 일본 자동차 | 글로벌 OEM        |
| BYDDY (BYD ADR)     | 중국 EV     | 배터리·EV 통합    |
| NIO                 | 중국 EV     | 프리미엄 EV       |
| XPEV (XPeng)        | 중국 EV     | 자율주행·EV       |
| LI (Li Auto)        | 중국 EV     | EREV·SUV          |

## Key Uncertainties (지속 추적)

본 섹션은 **분석 에이전트가 카드 만들 때 우선 참조** 하는 목록.

- EV vs ICE 전환 시점 (미국 IRA 정책 변동)
- 중국 EV (BYD·NIO·XPEV) 글로벌 침투 (유럽·동남아 점유율)
- 배터리 화학 (LFP vs NCM) 점유율 + LFP 우위 시나리오
- 자율주행 L4 상용화 (Tesla FSD vs Waymo vs Hyundai)
- 한국·일본·미국 자동차 관세 IEEPA (25% vs 협상 결과)
- 차량용 반도체 + AI 추론칩 (자동차 ↔ semiconductor 교차)
- 중고차 가격 + SAAR 미국 (월간 트래커)

## 카탈리스트 캘린더 (분기 단위)

| 분기    | 이벤트                       | 출처 유형  |
| ------- | ---------------------------- | ---------- |
| 월간    | 자동차 판매 (SAAR 미국)      | Policy     |
| 분기    | 어닝 (Tesla·GM·현대차)       | Filing     |
| 분기    | 중국 EV 수출 통계 (해관총서) | Policy     |
| 2026 Q1 | CES (1월)                    | Conference |
| 2026 Q3 | Munich Motor Show (9월 격년) | Conference |
| 분기    | NHTSA 자율주행 가이드라인    | Policy     |

## L1 인덱스 카운트 (현재)

- 학술: 1
- 씽크탱크: 0
- 컨퍼런스: 0
- 규제: 1

## L2 월간 요약 목록 (최신 → 과거)

- 2026-06 `ira_30d_industrial_policy_siepr_202606.md` — IRA 30D 클린비클 크레딧 = 신산업정책, 이원 $3,750 + 북미조립 요건, 미국 EV 제조 리쇼어링 함의 (SIEPR)

## L3 분기 Deep Dive 이력

- (없음 — 2026 Q3 첫 분기 발행 목표)

## 분석 에이전트 활용 가이드

분석 에이전트가 본 섹터 L2 요약 인용 시:

- EV 리더 (TSLA·RIVN) → IRA 정책 + 배터리 가격 L2
- 전통 자동차 (GM·F·TM·HMC) → SAAR 미국 + EV 전환 속도 L2
- 한국 자동차 (005380·000270) → IEEPA 관세 + 동남아 수출 L2
- 중국 EV (BYDDY·NIO·XPEV·LI) → 중국 점유율 + 유럽 침투 L2
- 자율주행 (TSLA FSD + Waymo + Hyundai) → 학술 (arXiv cs.RO) L2

## 관련 KB 교차 참조

- `knowledge-base/industry/auto.md` — 자동차 산업 thesis
- `knowledge-base/industry/battery.md` — 배터리 산업
- `knowledge-base/research/semiconductor/` — 차량용 반도체 교차
- `knowledge-base/research/macro/` — IEEPA 관세 교차
