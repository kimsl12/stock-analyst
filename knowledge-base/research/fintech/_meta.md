---
sector: fintech
created: 2026-05-12
last_updated: 2026-09-05
status: active
related_industry_kb:
  - knowledge-base/industry/fintech.md
  - knowledge-base/industry/financial_services.md
l1_index_count: 12
l2_summary_count: 8
l3_deep_dive_count: 1
deep_dive_priority: 5
---

# Research Meta — Fintech (핀테크)

> 모델 포트폴리오·내포트폴리오 추천 시 결제·디지털뱅킹·스테이블코인·크립토 ETF 노출 평가. BIS/SEC 1차 자료 직접 인용.

## 현재 thesis

- **SEC 가이드 명확화 전환점 (Atkins '10년 불확실성 해소')**: 3/17 SEC+CFTC 공동 5분류 token taxonomy (digital commodities/collectibles/tools/stablecoins/securities) + 4/13 셀프 호스팅 지갑 broker 미해당. 미국 등록 거래소 (COIN·Kraken) 우위 + 디지털 상품 spot ETF 추가 트랙 가시화. (L2: sec_cftc_token_taxonomy)
- **스테이블코인 시스템 리스크 BIS 공식 정량화**: BIS Papers #170 (5/5, Aldasoro·Frost·Ito) — 98% 달러 표시, 디지털 달러화 EMDE 통화주권 침식, 3 시나리오 (니치·달러화·통합). BIS WP #1340 — FX 스필오버 통계적 유의 (EM 통화 변동성). 거시 시장 변수 격상. (L2: stablecoin_bis_papers_170 / bis_wp_1340_stablecoin_fx)
- **글로벌 규제 단편화 + 차익 위험**: BIS Briefing 4/20 — 미국 GENIUS Act·EU MiCA·UK FCA·홍콩 HKMA·싱가포르 MAS 각각 진행되나 통합 표준 부재. 발행국·운영국·환매국 분절 시 cross-border 리스크 격상. (L2: bis_briefing_global_stablecoin_slowdown)

## Key Uncertainties

- 스테이블코인 시장 점유율 + 미국 단기국채 수요 채널 영향 (BIS WP 추적)
- BTC ETF spot 자금 유입 vs 유출 사이클 (IBIT 등 spot ETF)
- CBDC 도입 (EU 디지털유로, 중국 e-CNY 진행, 미국 FedNow)
- DeFi 시장 사이즈 + 규제 명확화 (SEC vs CFTC)
- BNPL (Buy Now Pay Later) 대출 사이클 — 부실률 + 규제
- 한국 토큰증권 (STO) 시장 개화 + 가상자산법

## 카탈리스트 캘린더

| 분기    | 이벤트                                | 출처 유형   |
| ------- | ------------------------------------- | ----------- |
| 분기    | SEC 가상자산 enforcement actions      | Policy      |
| 2026 Q3 | Money 20/20 (10월)                    | Conference  |
| 분기    | BIS Quarterly Review 핀테크 코너      | White Paper |
| 분기    | FedNow 누적 거래 통계                 | Policy      |
| 분기    | BIS Innovation Hub CBDC pilot updates | Think Tank  |

## L1 인덱스 카운트

- 학술: 2
- 씽크탱크: 1
- 컨퍼런스: 0
- 규제: 2

## L2 월간 요약 목록

- 2026-09 `cfpb_1033_open_banking_reconsideration_202609.md` — CFPB Section 1033 오픈뱅킹 규칙 E.D.Ky 법원 집행정지 + 2026-04-01 준수시한 무력화 + CFPB 재검토(2025-08 ANPRM), 2026-08-05 신규 NPRM OIRA 제출(데이터 접근 수수료 부과 허용 검토) → 스테이블코인과 별개 오픈뱅킹·데이터권 규제 축 (은행 vs 애그리게이터/핀테크 이해 충돌)
- 2026-08 `occ_genius_act_stablecoin_reserves_202608.md` — OCC NPRM(2/25) 스테이블코인 준비금 1:1(만기 93일 미만 국채)·유동성 일 10%/주 30%·집중 40%·상환 2영업일(급증 7일)·$25B↑ 예금보험·$10B 연방전환, 발효 2027-01-18
- 2026-06 `genius_act_sec_cftc_mou_custody_202606.md` — GENIUS Act 시행 본격화 (SEC·CFTC MOU 3/11 + 토큰증권 혁신 면제 + FDIC 패스스루 보험 배제 + OCC/Treasury 커스터디 룰, 7/18 데드라인)
- 2026-05 `stablecoin_bis_papers_170_202605.md` — BIS Papers #170 EMDE 통화주권 침식 3 시나리오
- 2026-04 `sec_self_custody_wallet_broker_202604.md` — SEC Staff Statement 4/13 셀프 호스팅 지갑 broker 미해당
- 2026-04 `bis_briefing_global_stablecoin_slowdown_202604.md` — BIS Briefing 4/20 단편화 위험
- 2026-03 `bis_wp_1340_stablecoin_fx_202603.md` — BIS WP #1340 스테이블코인 FX 스필오버
- 2026-03 `sec_cftc_token_taxonomy_202603.md` — SEC+CFTC 5분류 token taxonomy

## L3 분기 Deep Dive 이력

- 2026-05-12 `reports/research/fintech_2026Q2.html` — L3 #1 첫 발행 — BIS Papers #170 EMDE 침식 + SEC+CFTC 5분류 + 셀프 호스팅 staff statement (4/13) + BIS WP #1340 FX 스필오버 + BIS Briefing 단편화 통합 (5 L2 + financial_services KB 교차, 인용 15건 / debate 4 / contrarian 3 / 트래킹 10)
- (예정) `reports/research/fintech_2026Q3.html` — 2026-08 첫 일요일 발행

## 분석 에이전트 활용 가이드

- 결제 (V, MA, PYPL, SQ, ADYEY) → Money 20/20 + 결제량 통계 L2
- 디지털뱅킹 (NU, SOFI, COIN) → SEC + OCC 규제 L2
- 크립토 ETF (IBIT, FBTC, ETHA) → BIS 스테이블코인 + SEC 승인 동향 L2
- 한국 금융 (055550, 086790, 105560) → 금융위 토큰증권 + 가상자산법 L2
- ETF (KIE, FNTC) → 핀테크 사이클 NBER L2

## 관련 KB 교차 참조

- `knowledge-base/industry/fintech.md` (있을 시)
- `knowledge-base/industry/financial_services.md`
- `knowledge-base/macro/global_risk_factors.md` — 시스템 리스크
