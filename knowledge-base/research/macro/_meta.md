---
sector: macro
created: 2026-05-12
last_updated: 2026-09-05
status: active
related_industry_kb:
  - knowledge-base/macro/us_economy.md
  - knowledge-base/macro/us_monetary_policy.md
  - knowledge-base/macro/global_risk_factors.md
  - knowledge-base/macro/korea_economy.md
l1_index_count: 23
l2_summary_count: 8
l3_deep_dive_count: 1
deep_dive_priority: 3
---

# Research Meta — Macro (매크로)

> global-macro-analyst 의 G-1~G-8 분석을 학술·BIS·IMF 1차 자료로 강화. fred_snapshot.json 의 양적 데이터를 학술적 해석으로 보완.

## 현재 thesis

- **Fed 내 컨센서스 붕괴 + Powell 마지막 회의 가능성**: 4/29 FOMC 8-4 양방향 dissent (1992년 10월 이후 최대) — Miran 인하·Hammack/Kashkari/Logan 매파. 시장-Fed 정책 경로 괴리 격상. Powell 임기 만료 가능성으로 6/16-17이 후임 의장 첫 회의 가능. (L2: fomc_8_4_dissent_april)
- **IMF WEO 글로벌 성장 하향 + adverse 2.0% 시나리오**: 2026년 3.1%·2027년 3.2% (4/14), EM -0.3%p. "Global Economy in the Shadow of War" — adverse 2.5%·인플레 5.4%, severe 2.0%·인플레 6%+. EM·developing 피해는 advanced 2배. (L2: imf_weo_shadow_of_war)
- **시장 로테이션 가속 (M7 → 밸류·시클리컬)**: BIS QR 3월 "Markets recalibrate amid shifting currents" — 달러 약세 + M7·고모멘텀 테크 → 밸류/시클리컬, EME 강세 + 일본·호주 장기금리 급등. AI 단일 동력 thesis 첫 균열 신호. (L2: bis_qr_market_rotation)
- **인플레 vs 포용 정책 트레이드오프 학술 정량화**: NBER #34894 Alves & Violante — 매파 Fed가 저소득·소수자 노동시장 회복을 유의하게 지연. Fed dual mandate 해석에 학술 근거 + 차기 의장 인선 정당화 인용 가능성. (L2: nber_34894_inflation_inclusion)

## Key Uncertainties

- 미국 인플레이션 끈적함 (서비스물가) 지속 기간 — BIS/NBER 연구 기반
- Fed 정책금리 경로 (2026 말 3.625% dot plot vs 시장 OIS 곡선 괴리)
- 미국 재정 적자 + 국채 만기 wall (만기 차환 부담)
- 중국 디플레이션 + 부동산 사이클 + 위안화
- 일본 BOJ 정상화 (YCC 해제) 진행 + JGB 시장 영향
- EM 자본 흐름 (강달러 vs 약달러 사이클 전환)
- 탈달러화 진행 (CBDC + 금 + BRICS)

## 카탈리스트 캘린더

| 분기       | 이벤트                       | 출처 유형   |
| ---------- | ---------------------------- | ----------- |
| 2026 Q2~Q4 | FOMC 8회 (3·5·6·7·9·10·12월) | Policy      |
| 2026 8월   | Jackson Hole Symposium       | Conference  |
| 2026 6월   | BIS Annual Economic Report   | Conference  |
| 2026 Q3    | IMF Annual Meetings 가을     | Conference  |
| 분기       | BIS Quarterly Review         | White Paper |
| 분기       | IIF Capital Flows Tracker    | Think Tank  |

## L1 인덱스 카운트

- 학술: 2
- 씽크탱크: 2
- 컨퍼런스: 0
- 규제: 6

## L2 월간 요약 목록

- 2026-09 `warsh_jackson_hole_2026_202609.md` — Warsh 첫 Jackson Hole 기조 'In Our Time'(8/28) 매파 확정: PCE 12M 3.7%·6M 4.1% 물가 최우선 + "otherwise we have work to do"(조건부 추가 긴축) + 사이클 금리 quasi-commitment 명시적 거부(가이던스 축소·데이터 의존) + 심포지엄 테마 '금융혁신·결제·정책'. 의장 교체 후에도 Fed 매파 컨센서스 지속 확증
- 2026-08 `fomc_july_2026_hawkish_dissent_202608.md` — FOMC 7/29 5회 연속 동결(3.50~3.75%, 9-3) + 매파 반대 3인(Hammack·Kashkari·Logan 인상 선호) + 공급충격(에너지)·중동發 인플레 지속
- 2026-06 `bok_may_hold_inflation_revision_202606.md` — 한국은행 5/28 기준금리 2.50% 동결(8회 연속) + 2026 CPI 2.2→2.7%·GDP 2.0→2.6% 상향 (중동 유가+원화 약세)
- 2026-05 `boj_summary_april_2026_202605.md` — BOJ Summary of Opinions (4/27-28) 0.75% 동결 6-3 split + FY2026 Core CPI 1.9%→2.8% 상향
- 2026-04 `fomc_8_4_dissent_april_202604.md` — FOMC 4/29 8-4 dissent + Powell 마지막 회의 가능성
- 2026-04 `imf_weo_shadow_of_war_202604.md` — IMF WEO 4/14 글로벌 3.1% + adverse 2.0%
- 2026-03 `bis_qr_market_rotation_202603.md` — BIS QR 'Markets recalibrate' M7→밸류 로테이션
- 2026-04 `nber_34894_inflation_inclusion_202604.md` — NBER #34894 Alves & Violante 인플레 vs 포용

## L3 분기 Deep Dive 이력

- 2026-05-12 `reports/research/macro_2026Q2.html` — L3 #1 첫 발행 — Fed 8-4 dissent + IMF 3.1% + BOJ 매파적 동결 + BIS QR 로테이션 통합 (5 L2 + macro KB 교차, 인용 16건 / debate 4 / contrarian 3 / 트래킹 10)
- (예정) `reports/research/macro_2026Q3.html` — 2026-08 첫 일요일 발행

## 분석 에이전트 활용 가이드

- **global-macro-analyst 가 주력 활용** — G-1~G-8 의 각 섹션 인용 강화
- G-2 정책 → Fed FEDS + BIS WP + Jackson Hole 인용
- G-7 자본 흐름 → IIF + BIS QR 인용
- G-8 시나리오 → IMF WEO + BIS AER 시나리오 인용
- risk-analyst 의 "거짓 안정" / "침체 임박" 시그널 → BIS WP 으로 보강

## 관련 KB 교차 참조

- `knowledge-base/macro/us_economy.md` — 미국 경제 thesis
- `knowledge-base/macro/us_monetary_policy.md` — Fed 정책 트래커
- `knowledge-base/macro/global_risk_factors.md` — 글로벌 리스크
- `knowledge-base/macro/korea_economy.md` — 한국 경제
- `knowledge-base/macro/fred_snapshot.json` — FRED 1차 양적 데이터 (본 KB는 정성·시나리오 보완)
